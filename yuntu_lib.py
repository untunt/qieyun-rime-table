...
韻圖屬性列表 = [
    ...
    韻圖屬性類('　幽幽　尤侯', '　', 0, '流　摄', ' ɨʷ~ɨw'),  # 幽韻歸屬有爭議，畫韻圖時算作重紐韻。另外侯韻是開口
    韻圖屬性類('　　　　魚　', '開', 0, '遇　摄', ' əʷ'),
    韻圖屬性類('　　　　虞模', '　', 0, '遇　摄', 'wəʷ'),
    韻圖屬性類('　　　　之　', '開', 0, '止　摄', ' ɨ'),
    韻圖屬性類('　脂脂　微　', '開', 0, '止　摄', ' ɨʲ~ɨj'),
    韻圖屬性類('　脂脂　微　', '合', 0, '止　摄', 'wɨʲ~wɨj'),
    韻圖屬性類('佳支支　　　', '開', 0, '止蟹摄', ' əʲ'),
    韻圖屬性類('佳支支　　　', '合', 0, '止蟹摄', 'wəʲ'),
    韻圖屬性類('麻麻麻　歌歌', '開', 0, '果假摄', ' a'),
    韻圖屬性類('麻麻麻　歌歌', '合', 0, '果假摄', 'wa'),

    韻圖屬性類('　　　　東東', '　', 1, '通　摄', ' ɨŋʷ'),
    韻圖屬性類('江　　　鍾冬', '　', 1, '通江摄', ' əŋʷ'),
    韻圖屬性類('　蒸　　蒸登', '開', 1, '曾　摄', ' ɨŋ & əŋ'),  # 蒸韻歸屬有爭議，畫韻圖時算作重紐韻
    韻圖屬性類('　蒸　　　登', '合', 1, '曾　摄', 'wɨŋ & wəŋ'),
    韻圖屬性類('耕　　青　　', '開', 1, '梗　摄', ' əŋʲ'),
    韻圖屬性類('耕　　青　　', '合', 1, '梗　摄', 'wəŋʲ'),
    韻圖屬性類('庚庚清　　　', '開', 1, '梗　摄', ' aŋʲ'),
    韻圖屬性類('庚庚清　　　', '合', 1, '梗　摄', 'waŋʲ'),
    韻圖屬性類('　　　　陽唐', '開', 1, '宕　摄', ' aŋ'),
    韻圖屬性類('　　　　陽唐', '合', 1, '宕　摄', 'waŋ'),

    韻圖屬性類('皆祭祭齊廢咍', '開', 0, '蟹　摄', ' əj'),
    韻圖屬性類('皆祭祭齊廢灰', '合', 0, '蟹　摄', 'wəj'),
    韻圖屬性類('夬　　　　泰', '開', 0, '蟹　摄', ' aj'),
    韻圖屬性類('夬　　　　泰', '合', 0, '蟹　摄', 'waj'),

    韻圖屬性類('　眞眞　欣　', '開', 1, '臻　摄', ' ɨn'),  # 另外臻韻排在這張圖第一列（二等）
    韻圖屬性類('　眞眞　文　', '合', 1, '臻　摄', 'wɨn'),
    韻圖屬性類('山仙仙先元痕', '開', 1, '臻山摄', ' ən'),
    韻圖屬性類('山仙仙先元魂', '合', 1, '臻山摄', 'wən'),
    韻圖屬性類('刪　　　　寒', '開', 1, '山　摄', ' an'),
    韻圖屬性類('刪　　　　寒', '合', 1, '山　摄', 'wan'),

    韻圖屬性類('肴宵宵蕭　豪', '開', 0, '效　摄', ' əw~aw'),

    韻圖屬性類('　侵侵　　　', '開', 1, '深　摄', ' ɨm'),
    韻圖屬性類('咸鹽鹽添嚴覃', '開', 1, '咸　摄', ' əm'),  # 另外凡韻是合口，排在這張圖嚴韻一列
    韻圖屬性類('銜　　　　談', '開', 1, '咸　摄', ' am'),
]
...
def get音韻地位元組(圖號, 行號, 聲號, 列號):
    ...
    if is銳(母):
        ...
        if get組(母) == '莊' and not 韻:
            等 = '三'
        if 等 == '三':
            # 精組只能排在 AB 列和 C 列中的一列，另一列不應有韻目
            # 對於一張圖既有 AB 韻又有 C 韻的情況，精組只拼 AB 韻（尤韻除外）
            if get組(母) == '精':
                if (韻圖屬性.韻字典['A'] and 新等 == 'C' and 韻 != '尤') or 韻 == '幽':
                    韻 = ''
            # 精組以外的銳音三等可能是從 AB 或 C 變來的
            else:
...
def get圖號(音韻地位):
    ...
    if len(圖號列表) > 1:
        if 音韻地位.呼:
            ...
            圖號列表 = 圖號列表[-1:]
    if get組(音韻地位.母) == '莊' and 音韻地位.韻 in '祭廢仙元' and 音韻地位.呼 == '合':
        # 合口莊三化二進低元音
        return 圖號列表[0] + 2
    return 圖號列表[0]
